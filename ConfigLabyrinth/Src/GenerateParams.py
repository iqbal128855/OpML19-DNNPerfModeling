    ...
    def __init__(self,
        ...
        try:
            if self.sys_name=="TX1":
                ...
                self.file_name_output=self.output_file+"output_"+model_name+"_"+size+".csv"
                self.file_name_conf=self.output_file+"output_conf_"+model_name+"_"+size+".csv"
                self.file_name_invalid=self.output_file+"invalid_config_"+model_name+"_"+size+".csv"
        ...
        for conf in xrange(0,len(self.params)):
            ...
            for iteration in xrange(self.NUM_TEST):
                com_perf_ob=ComputePerformance(logger,
                                               cur_conf,
                                               self.sys_name,
                                               model,
                                               test_data)
                cur_conf_inference_time[iteration], cur_conf_power[iteration]=com_perf_ob.get_output_metrics()           
            # build output dataframe
            self.df.loc[conf]=[model_name,
                               sample,
                               size,
                               self.sys_name,
                               cur_conf_name,
                               ...
                               self.params[conf][7],
                               self.params[conf][8],                               
                               np.median(cur_conf_inference_time[1:]),                                
                               np.median(cur_conf_power[1:])]
            
            # dataframe showing the variation per iteration              
            self.dfc.loc[conf]=[model_name,
                               sample,
                               size,
                               self.sys_name,
                               cur_conf_name,
